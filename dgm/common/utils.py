...
def print_and_log_scalar(writer, name, value, write_no, end_token=""):
    ...
    zeros = 40 - len(name)
    name += " " * zeros
    print(f"{name} @ write {write_no} = {value:.4f}{end_token}")
...
def log_point_clouds(writer, data, name, step):
    ...
    out = np.stack([from_polar(x.transpose(1, 2, 0)) for x in data.cpu().data.numpy()])
...
def show_pc(velo, save_path=None):
    ...
    print(aa, bb)
    mayavi.mlab.view(
        azimuth=-87, elevation=-40, focalpoint=(0, 0, np.median(velo[:, -1]))
    )
    f = mayavi.mlab.gcf()
...
def load_model_from_file(path, epoch, model="dis"):
    ...
    from models import VAE, netD, netG
    ...
    model_.load_state_dict(
        torch.load(os.path.join(path, "models/%s_%d.pth" % (model, epoch)))
    )
