...
parser.add_argument(
    "--batch_size", type=int, default=64, help="size of minibatch used during training"
)
parser.add_argument(
...
for epoch in range(150 if args.autoencoder else 300):
    ...
    if epoch % 10 == 0:
        ...
        with open(os.path.join(args.base_dir, f"samples/train_{epoch}.npz"), "wb") as f:
            np.save(f, recon)
    ...
    with torch.no_grad():
        ...
        if epoch % 1 == 0:
            ...
            utils.print_and_log_scalar(writer, "valid/kl_cost_", mn(kl_cost_), writes)
